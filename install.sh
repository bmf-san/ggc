...
install_with_go() {
    ...
    if ! go install github.com/bmf-san/ggc/v6@latest; then
...
function __ggc_load_completion
    ...
    for completion_file in "$gopath"/pkg/mod/github.com/bmf-san/ggc/v6@*/tools/completions/ggc.fish
...
load_ggc_completion() {
	...
	for completion_file in "$gopath"/pkg/mod/github.com/bmf-san/ggc/v6@*/tools/completions/ggc.bash; do
...
load_ggc_completion() {
	...
	for completion_file in "$gopath"/pkg/mod/github.com/bmf-san/ggc/v6@*/tools/completions/ggc.zsh; do
		if [ -f "$completion_file" ]; then
			source "$completion_file"
			return 0
		fi
